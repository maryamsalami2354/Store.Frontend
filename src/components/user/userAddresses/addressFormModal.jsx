...
const AddressFormModal = ({ isOpen, address, onClose, onSave, isSaving = false }) => {
    const [formData, setFormData] = useState(emptyForm);

    useEffect(() => {
        setFormData(address ? { ...emptyForm, ...address } : emptyForm);
    }, [address, isOpen]);

    const handleChange = (e) => {
        const { name, value, type, checked } = e.target;
        const nextValue = ['postalCode', 'plateNumber', 'unitNumber'].includes(name)
            ? onlyDigits(value, name === 'postalCode' ? 10 : 10)
            : value;
    ...
    const handleSubmit = async (e) => {
        ...
        if (formData.postalCode && !/^\d{10}$/.test(formData.postalCode.trim())) {
            toast.error('کد پستی باید ۱۰ رقم باشد');
            return;
        }

        if (formData.plateNumber && !/^\d+$/.test(formData.plateNumber.trim())) {
            toast.error('شماره پلاک باید عددی باشد');
            return;
        }

        if (formData.unitNumber && !/^\d+$/.test(formData.unitNumber.trim())) {
            toast.error('شماره واحد باید عددی باشد');
            return;
        }
                ...
                <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm" onClick={onClose}>
                    <motion.div initial={{ scale: 0.9 }} animate={{ scale: 1 }} exit={{ scale: 0.9 }} className="relative w-full max-w-lg max-h-[90vh] overflow-y-auto bg-white dark:bg-[#111] rounded-2xl shadow-2xl" onClick={e => e.stopPropagation()}>
                        <div className="p-5 border-b border-gray-200 dark:border-gray-800 flex items-center justify-between">
                            ...
                            <button type="button" onClick={onClose} className="p-2 rounded-lg hover:bg-gray-100 dark:hover:bg-gray-800"><X size={20} className="text-gray-500" /></button>
                        </div>
                        <form onSubmit={handleSubmit} className="p-5 space-y-4">
                            ...
                            <div>
                                ...
                                <textarea name="address" value={formData.address} onChange={handleChange} rows={2} placeholder="خیابان، کوچه، نام ساختمان یا توضیحات مسیر" className="w-full p-2.5 rounded-xl border border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-900 text-sm focus:ring-2 focus:ring-[#002874]" />
                            </div>
                            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                                <div>
                                    <label className="block text-sm font-medium mb-1.5">شماره پلاک</label>
                                    <input name="plateNumber" value={formData.plateNumber} onChange={handleChange} placeholder="مثال: ۱۲" inputMode="numeric" className="w-full p-2.5 rounded-xl border border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-900 text-sm dir-ltr text-left focus:ring-2 focus:ring-[#002874]" />
                                </div>
                                <div>
                                    <label className="block text-sm font-medium mb-1.5">شماره واحد</label>
                                    <input name="unitNumber" value={formData.unitNumber} onChange={handleChange} placeholder="مثال: ۴" inputMode="numeric" className="w-full p-2.5 rounded-xl border border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-900 text-sm dir-ltr text-left focus:ring-2 focus:ring-[#002874]" />
                                </div>
                            </div>
                            ...
                            <div className="flex gap-3 pt-3">
                                <button type="button" onClick={onClose} disabled={isSaving} className="flex-1 py-2.5 rounded-xl border border-gray-200 dark:border-gray-700 text-gray-700 dark:text-gray-300 hover:bg-gray-50 dark:hover:bg-gray-800 transition disabled:opacity-60">انصراف</button>
                                <button type="submit" disabled={isSaving} className="flex-1 py-2.5 rounded-xl bg-[#002874] text-white font-medium flex items-center justify-center gap-2 hover:bg-[#001d5a] transition disabled:opacity-60">
                                    <Save size={16} />
